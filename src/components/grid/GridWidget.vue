<template>
    <PageMain :title="trans.title" :has-filter="get(trans.filter, 'items', []).length > 0" :in-page="!interactive"
        ...
        <!--范围-->
        <GridScope :scope="trans.scope" :scopes="trans.scopes" @update:scope="onUpdateScope"/>
        <!--搜索-->
        <GridSearch v-show="trans.isFilterVisible" :attr="trans.filter" :pk="trans.pk" :pk-values="trans.pkValues"
            @search="onSearch" @export="onExport"
        />
        <!--批处理-->
...
const combineQuery = (page: null | number, page_size: null | number, params: {} | null, sort: {} | null = null) => {
   ...
   * ---------------------------------------- */
    ...
    // 是否需要更新路由信息
    if (!isEqual(routeCoverEncoded, query) && props.interactive) {
        nextTick(() => {
            router.push({
...
const onExport = (val: any) => {
    ...
    console.log('url', trans.url, query, url);
    return;
